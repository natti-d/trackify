...
/*Проверка, ако ще се назначава задача*/
...
/*В противен случай се изтрива задачата*/
}else if($_POST['action'] = 'Delete'){
    $command = "DELETE FROM `Members` WHERE `task_id`='$task';";
    $deleteTaskFromMemb = mysqli_query($conn, $command);
    if ($deleteTaskFromMemb) {}
    $command = "DELETE FROM `Tasks` WHERE `task_id`='$task';";
    $deleteTaskFromTasks = mysqli_query($conn, $command);
    if ($deleteTaskFromTasks) {}
}
/*Пренасочване*/
echo "<script>location.href='../loaded_project.php'; localStorage.removeItem('taskID');</script>"
?>
